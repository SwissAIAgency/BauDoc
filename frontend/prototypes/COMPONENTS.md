...
| 2026-06-23  | preview-card-spec.css, docs/ux/concepts/preview-card-spec.md | Serpentine-Routing überarbeitet: `--preview-serpentine-curve-radius` 75→32px; `--preview-serpentine-side-offset` deprecated (Offset jetzt dynamisch `cellHalf + 36px` in `updateSerpentinePath()`); neue Vars `--gal-curve-pad` 72px + `--gal-cols` 6 für dynamisches Galerie-Grid | Timeline lief über die Bilder statt daneben. Dot ist zellzentriert → fixer Offset kann bei variabler Spaltenzahl nicht „nah am Bild UND daneben" sein. Dynamische Berechnung aus gemessener Bildbreite hält die Linie bei jeder Spaltenzahl mittig in der 36px-Padding-Lücke (symmetrisch Bild↔Linie↔Rand). Freigabe: Joshua, 2026-06-23 |
